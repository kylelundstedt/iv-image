...
| File            | Role                                                                   |
| --------------- | ---------------------------------------------------------------------- |
| `Dockerfile.iv` | The IV overlay: pinned exeuntu base + DuckDB + Quarto + doc-site tooling + Tailscale join service. |
| `build.sh`      | Fetch pinned exeuntu, build base + overlay, tag, push, record digest.  |
| `bin/`          | `render-site` + `provision-docsite` + `gen-llms-txt` + `iv-tailscale-join` — baked onto PATH in the image. |
